...
def _fmt_hex(value, hex_len=0):
    if isinstance(value, str):
        v = value.strip()
        neg = v.startswith('-')
        if neg:
            v = v[1:].lstrip()
        v = re.sub(r'^(?:0[xX]|[xX])+', '', v)
        value = int(v, 16)
        if neg:
            value = -value
    sign = '-' if value < 0 else ''
    return f'{sign}0x{hex(abs(value))[2:].zfill(hex_len).upper()}'
...
class Enum(c_ast.Enum):
    ...
    def to_dict(self):
        ...
        hex_len = len(hex(abs(value_num))[2:])
        for member in members:
            member['value'] = _fmt_hex(member['value'], hex_len)
