...
def method_name_from_func_name(func_name):
    ...
    if '_del' in func_name:
        func_name = func_name.replace('_del', '_delete')
...
def get_enum_name(enum):
    ...
    return enum.replace('LV_', '').replace('ENUM_', '')
...
for enum_def in enum_defs:
    ...
    for member in enum_def.type.values.enumerators:
        ...
        enums[member_name] = 'MP_ROM_INT(%s)' % member.name
...
for enum_def in enum_defs:
    if not enum_def.type.values:
        continue
    member_names = [member.name for member in enum_def.type.values.enumerators if lv_str_enum_pattern.match(member.name)]
    # enum_name = commonprefix(member_names)
    # enum_name = "_".join(enum_name.split("_")[:-1]) # remove suffix
    enum = collections.OrderedDict()
    for member_name in member_names:
        # full_name = str_enum_to_str()
        print('MP_DEFINE_STR_OBJ(mp_%s, %s);' % (member_name, member_name))
        enums[member_name] = '&mp_%s' % member_name
...
def try_generate_struct(struct_name, struct):
    ...
    for decl in flatten_struct_decls:
        ...
        if callback:
            ...
            read_cases.append('case MP_QSTR_{field}: dest[0] = (mp_obj_t)mp_lv_funcptr(&mp_{funcptr}_mpobj, {cast}data->{field}, {lv_callback} ,MP_QSTR_{struct_name}_{field}, {user_data}); break; // converting from callback {type_name}'.
            ...
            if isinstance(decl.type, c_ast.ArrayDecl):
                ...
                read_cases.append('case MP_QSTR_{field}: dest[0] = (mp_obj_t){convertor}({cast}data->{field}); break; // converting from {type_name}'.
                ...
                read_cases.append(
                    'case MP_QSTR_{field}: dest[0] = (mp_obj_t){convertor}({cast}data->{field}); break; // converting from {type_name}'.
                        format(field=sanitize(decl.name), convertor=lv_to_mp_convertor, type_name=type_name, cast=cast))
...
static inline mp_obj_t mp_read_ptr_{sanitized_struct_name}(const {struct_name} *field)
{{
...
def build_callback_func_arg(arg, index, func, func_name = None):
    ...
    cast_in = '(void *)' if isinstance(arg.type, c_ast.PtrDecl) and 'const' in get_type(arg.type) and 'void' in get_type(arg.type) else '' # needed when field is const. casting to void overrides it
    cast_out = f'({arg_type})'
    ...
    return 'mp_args[{i}] = (mp_obj_t){convertor}({cast_in}arg{i});'.format(
                convertor = lv_to_mp[arg_type],
                i = index, cast_in = cast_in)
...
def build_mp_func_arg(arg, index, func, obj_name):
    ...
    cast_out = f'({gen.visit(fixed_arg.type)})'

    return '{var} = {cast_out}{convertor}(mp_args[{i}]);'.format(
            var = gen.visit(fixed_arg),
            convertor = mp_to_lv[arg_type],
            i = index, cast_out=cast_out)
...
{{
    ...
    return (mp_obj_t){build_return_value};
}}
...
for (func_name, func, struct_name) in callbacks_used_on_structs:
    try:
        ...
        # struct_metadata[struct_name]['methods'][func_name] = deepcopy(callback_metadata[func.name])
    except MissingConversionException as exp:
        gen_func_error(func, exp)
        # func_name = get_arg_name(func.type)
        # lv_to_mp[func_name] = lv_to_mp['void *']
...
""".format(
        ...
        enums = ''.join(['{{ MP_ROM_QSTR(MP_QSTR_{name}), MP_ROM_PTR({enum}) }},\n    '.
            format(name = sanitize(get_enum_name(enum_name)), enum=enums[enum_name]) for enum_name in enums.keys()]),
...
for obj_name in obj_names:
    ...
    obj_metadata[obj_name]['methods'].update(deepcopy(obj_metadata[obj_name]['members']))
